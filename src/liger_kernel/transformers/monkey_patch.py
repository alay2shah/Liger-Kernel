...
def apply_liger_kernel_to_deepseek_v4(
    ...
    if fused_linear_cross_entropy:
        if model is not None:
            model.forward = MethodType(deepseek_v4_lce_forward, model)
        else:
...
def _use_lfm2_vl_selective_defaults() -> bool:
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        ...
        try:
            ...
            world_size = 1
    if world_size < 2 or not torch.cuda.is_available() or torch.version.hip is not None:
        return False
    properties = torch.cuda.get_device_properties(torch.cuda.current_device())
    return properties.major == 9 and properties.minor == 0 and "H100" in properties.name.upper()
